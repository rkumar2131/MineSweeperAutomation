package com.ravi.minesweeper.apptest.util;
...
public class ObjectRepository {
	...
	public static String getElementProperty(final String elementName, final String propertyType) {
		if (doc == null) {
			documentBuilder();
		}
		...
		for (int nodeIndex = 0; nodeIndex < nodes.getLength(); nodeIndex++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE
					&& ((Element) node).getAttribute(tagAttribute).equals(
							elementName)) {
				return ((Element) node).getElementsByTagName(propertyType).item(0)
						.getTextContent();
			}
		}
		return null;
